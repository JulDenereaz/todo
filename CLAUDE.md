...
- The `activity` table records notable events (task created/updated/deleted, member added/removed, list renamed) scoped to a `listId`. `GET /api/activity` returns the feed across all lists the caller can access (or one list via `?listId=`), same "optional listId, else all accessible lists" pattern as `/api/tasks`.
- Writes happen server-side as a side effect inside the existing mutation route handlers (`logActivity()` from `src/lib/activity.ts`) — there's no separate client-facing write endpoint. Task edits are all one `task_updated` event per PATCH request (not one event per field) — `src/app/api/tasks/[taskId]/route.ts` diffs each changed field against the pre-update row into a `changes: string[]` (e.g. `"marked as done"`, `"set priority to high"`) and joins them into one summary, so unrelated fields changing together in the same request read as a single entry instead of a burst of near-duplicates. A PATCH that changes nothing logs nothing.
...
- When the user says to merge/ship/release, merge `dev` into `main` with `--no-ff` (even when a fast-forward is possible) and a merge commit message summarizing what's included — a short bullet list of the notable feature/fix commits since the last merge, not the default "Merge branch 'dev' into main". Then push `main`. `dev` itself can be pushed anytime — it doesn't trigger the pipeline.
- The release pipeline (`.github/workflows/build.yml`) builds each GitHub Release's notes from the non-merge commits between the previous and new tag — keep individual commits on `dev` scoped and descriptive (per the rule above) since their messages end up in the release changelog verbatim.
